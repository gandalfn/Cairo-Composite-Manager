...
G_DEFINE_TYPE (CCMConfigColorButton, ccm_config_color_button, GTK_TYPE_COLOR_BUTTON);
...
ccm_config_color_button_finalize (GObject *object)
{
	...
	if (self->priv->config_alpha) g_object_unref(self->priv->config_alpha);
	if (self->priv->key) g_free(self->priv->key);
	if (self->priv->key_alpha) g_free(self->priv->key_alpha);
...
static void
ccm_config_color_button_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
	...
	{
		if (self->priv->config_alpha) g_object_unref(self->priv->config_alpha);
		self->priv->config_alpha = ccm_config_new(self->priv->screen, 
		                                          self->priv->plugin,
		                                          self->priv->key_alpha);
...
static void
ccm_config_color_button_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	g_return_if_fail (CCM_IS_CONFIG_COLOR_BUTTON (object));
