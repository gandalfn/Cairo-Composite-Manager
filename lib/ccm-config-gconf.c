...
static gboolean ccm_config_gconf_get_boolean (CCMConfig * config,
                                              GError ** error);
...
static GSList *ccm_config_gconf_get_string_list (CCMConfig * config,
                                                 GError ** error);
static void ccm_config_gconf_set_string_list (CCMConfig * config,
                                              GSList * value, GError ** error);

static GSList *ccm_config_gconf_get_integer_list (CCMConfig * config,
                                                  GError ** error);
static void ccm_config_gconf_set_integer_list (CCMConfig * config,
                                               GSList * value, GError ** error);
...
ccm_config_gconf_copy_entry (CCMConfigGConf * self, gchar * src)
{
    ...
    entry =
        gconf_client_get_entry (CCM_CONFIG_GCONF_GET_CLASS (self)->client, src,
                                NULL, TRUE, NULL);
    if (entry && entry->value
        && gconf_engine_associate_schema (CCM_CONFIG_GCONF_GET_CLASS (self)->
                                          client->engine, self->priv->key,
...
static gboolean
ccm_config_gconf_initialize (CCMConfig * config, int screen, gchar * extension,
                             gchar * key)
{
    ...
    if (screen >= 0)
    {
        if (extension)
        {
            ...
            self->priv->key =
                g_strdup_printf ("%s/screen_%i/%s/%s", CCM_CONFIG_GCONF_PREFIX,
                                 screen, extension, key);

            entry =
                gconf_client_get_entry (CCM_CONFIG_GCONF_GET_CLASS (self)->
                                        client, self->priv->key, NULL, TRUE,
                                        NULL);
            if (!entry || !entry->value)
            {
        ...
        else
        {
            GConfEntry *entry;
            self->priv->key =
                g_strdup_printf ("%s/screen_%i/general/%s",
    ...
    self->priv->id_notify =
        gconf_client_notify_add (CCM_CONFIG_GCONF_GET_CLASS (self)->client,
                                 self->priv->key,
                                 (GConfClientNotifyFunc)
                                 ccm_config_gconf_on_change, self, NULL, NULL);
...
ccm_config_gconf_get_value_type (CCMConfig * config, GError ** error)
{
    ...
    value =
        gconf_client_get (CCM_CONFIG_GCONF_GET_CLASS (self)->client,
                          self->priv->key, error);
    if (value)
    {
...
static void
ccm_config_gconf_set_boolean (CCMConfig * config, gboolean value,
                              GError ** error)
{
    CCMConfigGConf *self = CCM_CONFIG_GCONF (config);
                                   ...
                                   GError ** error)
{
    ...
    gconf_client_set_list (CCM_CONFIG_GCONF_GET_CLASS (self)->client,
                           self->priv->key, GCONF_VALUE_INT, (GSList *) value,
                           error);
}
