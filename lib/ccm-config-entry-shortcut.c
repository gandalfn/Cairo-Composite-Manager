...
static gboolean ccm_config_entry_shortcut_focus_out_event(GtkWidget* widget, 
                                                          GdkEventFocus *event);
static gboolean ccm_config_entry_shortcut_button_press_event(GtkWidget* widget, 
...
ccm_config_entry_shortcut_finalize (GObject *object)
{
	...
	if (self->priv->old_value) g_free(self->priv->old_value);
...
static void
ccm_config_entry_shortcut_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
static void
ccm_config_entry_shortcut_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	g_return_if_fail (CCM_IS_CONFIG_ENTRY_SHORTCUT (object));
                                       ...
                                       GdkEventScroll* event)
{
	...
	if (GTK_WIDGET_CLASS(ccm_config_entry_shortcut_parent_class)->scroll_event)
		ret = GTK_WIDGET_CLASS(ccm_config_entry_shortcut_parent_class)->scroll_event(widget, event);
...
GtkWidget*
ccm_config_entry_shortcut_new (gboolean mouse, gint screen, 
