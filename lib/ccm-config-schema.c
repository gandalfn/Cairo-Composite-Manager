...
ccm_config_schema_finalize (GObject *object)
{
	...
	if (self->priv->file) g_key_file_free(self->priv->file);
...
ccm_config_schema_new (int screen, gchar* name)
{
	...
	if (!g_key_file_load_from_file(self->priv->file, self->priv->filename, 
...
ccm_config_schema_new_from_file (int screen, gchar* filename)
{
	...
	if (!g_key_file_load_from_file(self->priv->file, filename, 
	                               G_KEY_FILE_KEEP_COMMENTS | 
	                               G_KEY_FILE_KEEP_TRANSLATIONS,
	                               &error))
	{
...
ccm_config_schema_get_value_type(CCMConfigSchema* self, gchar* key)
{
	...
	else if (!g_ascii_strcasecmp(CCM_CONFIG_VALUE_TYPE_NAME[CCM_CONFIG_VALUE_LIST],
	                    		 type))
	{
...
gchar*
ccm_config_schema_get_description(CCMConfigSchema* self, gchar* locale, gchar* key)
{
...
ccm_config_schema_get_default(CCMConfigSchema* self, gchar* key)
{
	...
	return g_key_file_get_string(self->priv->file, key, 
	                             CCM_CONFIG_SCHEMA_ENTRY_DEFAULT,
	                             NULL);
}
...
ccm_config_schema_write_gconf(CCMConfigSchema* self, gchar* filename)
{
	...
	GError* error = NULL;
	gboolean ret = FALSE;
	gchar* top = g_markup_printf_escaped("<?xml version=\"1.0\"?>\n"
	                                     "<gconfschemafile>\n"
	                                     "  <schemalist>\n");
	gchar* bottom = g_markup_printf_escaped("  </schemalist>\n"
	                                        "</gconfschemafile>\n");
	...
	for (cpt = 0; groups && groups[cpt]; cpt++)
	{
		...
		{
			key = g_markup_printf_escaped("    <schema>\n"
			                              "      <key>/schemas%s/%s/%s</key>\n",
			                              CCM_CONFIG_GCONF_PREFIX,
			                              CCM_CONFIG_SCHEMA_GENERAL_PATH, 
			                              groups[cpt]);
			applyto = g_markup_printf_escaped("      <applyto>%s/%s/%s</applyto>\n",
			                                  CCM_CONFIG_GCONF_PREFIX, 
			                                  CCM_CONFIG_SCHEMA_GENERAL_PATH,
			                                  groups[cpt]);
		}
		else
		{
			...
			applyto = g_markup_printf_escaped("      <applyto>%s/%s/%s/%s</applyto>\n",
			                                  CCM_CONFIG_GCONF_PREFIX, 
			                                  CCM_CONFIG_SCHEMA_SCREEN_PATH,
			                                  self->priv->name, groups[cpt]);
		}
		...
		{
			type = g_markup_printf_escaped("      <type>%s</type>\n",
		...
		else if (t > CCM_CONFIG_VALUE_LIST)
		{
			...
			tmp = g_markup_printf_escaped("      <type>%s</type>\n",
			                CCM_CONFIG_VALUE_TYPE_NAME[CCM_CONFIG_VALUE_LIST]);
			if (t == CCM_CONFIG_VALUE_LIST_BOOLEAN)
			{
				type_list = 
					g_markup_printf_escaped("      <list_type>%s</list_type>\n",
		...
		{
			if (t > CCM_CONFIG_VALUE_INVALID && t < CCM_CONFIG_VALUE_LIST)
				defaut = g_markup_printf_escaped("      <default>%s</default>\n",
				                                 tmp);
			else  if (t > CCM_CONFIG_VALUE_LIST)
				defaut = g_markup_printf_escaped("      <default>[%s]</default>\n",
				                                 tmp);
		...
		if (tmp)
		{
			description = g_markup_printf_escaped("      <locale name=\"C\">\n"
			                                      "        <short>%s</short>\n"
			                                      "        <long>%s</long>\n"
			                                      "      </locale>\n",
			                                      tmp, tmp);
		...
		if (!body)
		{
			body = g_strconcat(key, applyto, owner, type, 
			                   defaut, description, "    </schema>\n", NULL);
		}
		else
		{
			tmp = body;
			body = g_strconcat(tmp, key, applyto, owner, type, 
			                   defaut, description, "    </schema>\n", NULL);
			g_free(tmp);
		}
		g_free(key);
