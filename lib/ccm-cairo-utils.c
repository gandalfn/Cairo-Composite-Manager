...
cairo_surface_t* 
cairo_image_surface_blur2 (cairo_surface_t *surface,
						   double           radius,
						   int			    x_orig,
						   int			    y_orig,
						   int			    width,
        ...
        for (channel = 0; channel < nchan; channel++) 
		{
            ...
            for (y = radius; y < h - radius; y++) 
			{
                for (x = radius; x < w - radius; x++) 
				{
                    ...
                    int tot = precalc[r+b*w] + precalc[l+t*w] - 
                        precalc[l+b*w] - precalc[r+t*w];
