...
static gboolean ccm_config_key_get_boolean (CCMConfig * config,
                                            GError ** error);
...
static void
ccm_config_key_monitor_changed (CCMConfigKeyMonitor * self, GFile * file,
                                GFile * other_file,
...
ccm_config_key_finalize (GObject * object)
{
    ...
    if (self->priv->schema_name)
        g_free (self->priv->schema_name);
    if (self->priv->key)
...
static gboolean
ccm_config_key_initialize (CCMConfig * config, int screen, gchar * extension,
                           gchar * key)
{
    ...
    if (!self->priv->monitor)
    {
        ...
        g_hash_table_insert (CCM_CONFIG_KEY_GET_CLASS (self)->configs, filename,
                             self->priv->monitor);
    }
    else
        g_free (filename);
...
ccm_config_key_set_boolean (CCMConfig * config, gboolean value, GError ** error)
{
    ...
    g_key_file_set_boolean (self->priv->monitor->keyfile, self->priv->name,
                            self->priv->key, value);
    ccm_config_key_monitor_sync (self->priv->monitor);
}
...
ccm_config_key_set_integer (CCMConfig * config, gint value, GError ** error)
{
    ...
    g_key_file_set_integer (self->priv->monitor->keyfile, self->priv->name,
                            self->priv->key, value);
    ccm_config_key_monitor_sync (self->priv->monitor);
}
...
ccm_config_key_set_float (CCMConfig * config, gfloat value, GError ** error)
{
    ...
    g_key_file_set_double (self->priv->monitor->keyfile, self->priv->name,
                           self->priv->key, value);
    ccm_config_key_monitor_sync (self->priv->monitor);
}
