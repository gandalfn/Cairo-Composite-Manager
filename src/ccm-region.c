...
_ccm_region_print (CCMRegion * self)
{
    ...
    ccm_log ("REGION: %f,%f %f,%f", clipbox.x, clipbox.y, clipbox.width,
             clipbox.height);
                           ...
                           gint * n_rectangles)
{
    ...
    {
        *rectangles =
            g_slice_alloc (sizeof (cairo_rectangle_t) * *n_rectangles);
...
ccm_region_point_in (CCMRegion * self, int x, int y)
{
    ...
    return pixman_region32_contains_point (&self->reg, pixman_int_to_fixed (x),
                                           pixman_int_to_fixed (y), &box);
}
