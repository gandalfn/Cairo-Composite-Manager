...
static void
ccm_cursor_set_property (GObject *object, guint prop_id, 
...
static void
ccm_cursor_get_property (GObject *object, guint prop_id, 
...
CCMCursor*
ccm_cursor_new (CCMDisplay* display, gchar* name)
{
	g_return_val_if_fail (display != NULL, NULL);
	g_return_val_if_fail (name != NULL, NULL);

	CCMCursor* self = NULL;
	gchar* theme = XcursorGetTheme (CCM_DISPLAY_XDISPLAY (display));
	int size = XcursorGetDefaultSize (CCM_DISPLAY_XDISPLAY (display));
	XcursorImage* cursor = XcursorLibraryLoadImage (name, theme, size);
	...
	if (cursor)
	{
		...
		ccm_debug("CURSOR %s", name);
		self = g_object_new(CCM_TYPE_CURSOR, 
		                    "x-hot", cursor->xhot,
		                    "y-hot", cursor->yhot, NULL);
		...
		self->priv->surface = cairo_image_surface_create_for_data (
												...
												cursor->width * 4);
		cairo_surface_set_user_data (self->priv->surface, &data_key, 
		                             image, g_free);
