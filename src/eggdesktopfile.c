                                     ...
                                     GError ** error)
{
    ...
    key_file = g_key_file_new ();
    if (!g_key_file_load_from_data_dirs
        (key_file, desktop_file_path, &full_path, 0, error))
    {
        g_key_file_free (key_file);
        return NULL;
    }
    ...
    desktop_file =
        egg_desktop_file_new_from_key_file (key_file, full_path, error);
    g_free (full_path);
    return desktop_file;
}
...
egg_desktop_file_new_from_dirs (const char *desktop_file_path,
                                const char **search_dirs, GError ** error)
{
    EggDesktopFile *desktop_file;
    GKeyFile *key_file;
    char *full_path;
    ...
    key_file = g_key_file_new ();
    if (!g_key_file_load_from_dirs
        (key_file, desktop_file_path, search_dirs, &full_path, 0, error))
    {
        g_key_file_free (key_file);
        return NULL;
    }
 ...
 **/
EggDesktopFile *
egg_desktop_file_new_from_key_file (GKeyFile * key_file, const char *source,
                                    GError ** error)
{
    EggDesktopFile *desktop_file;
    char *version, *type;
    ...
    version =
        g_key_file_get_value (key_file, EGG_DESKTOP_FILE_GROUP,
                              EGG_DESKTOP_FILE_KEY_VERSION, NULL);
    if (version)
    {
        double version_num;
        char *end;
        ...
        if (*end)
        {
            g_warning ("Invalid Version string '%s' in %s", version,
    ...
    desktop_file->name =
        g_key_file_get_string (key_file, EGG_DESKTOP_FILE_GROUP,
    ...
    if (!strcmp (type, "Application"))
    {
        ...
        exec =
            g_key_file_get_string (key_file, EGG_DESKTOP_FILE_GROUP,
                                   EGG_DESKTOP_FILE_KEY_EXEC, error);
        if (!exec)
        {
            egg_desktop_file_free (desktop_file);
            g_free (type);
    ...
    else if (!strcmp (type, "Link"))
    {
        ...
        url =
            g_key_file_get_string (key_file, EGG_DESKTOP_FILE_GROUP,
                                   EGG_DESKTOP_FILE_KEY_URL, error);
        if (!url)
        {
            egg_desktop_file_free (desktop_file);
            g_free (type);
    ...
    desktop_file->icon =
        g_key_file_get_string (key_file, EGG_DESKTOP_FILE_GROUP,
                               EGG_DESKTOP_FILE_KEY_ICON, NULL);
    if (desktop_file->icon && !g_path_is_absolute (desktop_file->icon))
    {
                ...
                || !strcmp (ext, ".svg")))
        {
            g_warning ("Desktop file '%s' has malformed Icon key '%s'"
                       ...
                       source ? source : "(unknown)", desktop_file->icon);
            *ext = '\0';
        }
    }
...
gboolean
egg_desktop_file_has_key (EggDesktopFile * desktop_file, const char *key,
                          GError ** error)
{
    return g_key_file_has_key (desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
                               key, error);
}
                             ...
                             const char *desktop_environment)
{
    ...
    {
        only_show_in =
            g_key_file_get_string_list (desktop_file->key_file,
                                        EGG_DESKTOP_FILE_GROUP,
                                        EGG_DESKTOP_FILE_KEY_ONLY_SHOW_IN, NULL,
                                        NULL);
        if (only_show_in)
        {
        ...
        not_show_in =
            g_key_file_get_string_list (desktop_file->key_file,
                                        EGG_DESKTOP_FILE_GROUP,
                                        EGG_DESKTOP_FILE_KEY_NOT_SHOW_IN, NULL,
                                        NULL);
        if (not_show_in)
        {
    ...
    if (desktop_file->type == EGG_DESKTOP_FILE_TYPE_APPLICATION)
    {
        try_exec =
            g_key_file_get_string (desktop_file->key_file,
                                   EGG_DESKTOP_FILE_GROUP,
                                   EGG_DESKTOP_FILE_KEY_TRY_EXEC, NULL);
        if (try_exec)
        {
...
static void
append_quoted_word (GString * str, const char *s, gboolean in_single_quotes,
...
static void
do_percent_subst (EggDesktopFile * desktop_file, char code, GString * str,
                  GSList ** documents, gboolean in_single_quotes,
                  gboolean in_double_quotes)
{
    GSList *d;
    char *doc;
...
static char *
parse_exec (EggDesktopFile * desktop_file, GSList ** documents, GError ** error)
{
    char *exec, *p, *command;
    ...
    exec =
        g_key_file_get_string (desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
                               EGG_DESKTOP_FILE_KEY_EXEC, error);
    if (!exec)
        return NULL;
    ...
    for (p = exec; *p != '\0'; p++)
    {
        ...
        {
            do_percent_subst (desktop_file, p[1], gs, documents, single_quot,
                              double_quot);
            p++;
        }
        else
    ...
    if (g_key_file_has_key
        (desktop_file->key_file, EGG_DESKTOP_FILE_GROUP,
         EGG_DESKTOP_FILE_KEY_TERMINAL, NULL))
    {
        GError *terminal_error = NULL;
        gboolean use_terminal = g_key_file_get_boolean (desktop_file->key_file,
                                                        EGG_DESKTOP_FILE_GROUP,
...
static gboolean
parse_link (EggDesktopFile * desktop_file, EggDesktopFile ** app_desktop_file,
            GSList ** documents, GError ** error)
{
    ...
    *documents = g_slist_prepend (NULL, url);
    ...
    g_key_file_set_string (key_file, EGG_DESKTOP_FILE_GROUP,
                           EGG_DESKTOP_FILE_KEY_NAME, "xdg-open");
    g_key_file_set_string (key_file, EGG_DESKTOP_FILE_GROUP,
                           EGG_DESKTOP_FILE_KEY_TYPE, "Application");
    g_key_file_set_string (key_file, EGG_DESKTOP_FILE_GROUP,
                           EGG_DESKTOP_FILE_KEY_EXEC, "xdg-open %u");
    *app_desktop_file =
        egg_desktop_file_new_from_key_file (key_file, NULL, NULL);
    return TRUE;
}
...
static void
end_startup_notification (GdkDisplay * display, const char *startup_id)
{
    gdk_x11_display_broadcast_startup_message (display, "remove", "ID",
                                               startup_id, NULL);
}
...
static void
set_startup_notification_timeout (GdkDisplay * display, const char *startup_id)
{
...
static gboolean
egg_desktop_file_launchv (EggDesktopFile * desktop_file, GSList * documents,
                          va_list args, GError ** error)
{
    ...
    do
    {
        ...
        startup_id =
            start_startup_notification (display, desktop_file, argv[0],
                                        screen_num, workspace, launch_time);
        if (startup_id)
        {
        ...
        current_success =
            g_spawn_async_with_pipes (directory, argv,
                                      env ? (char **) (env->pdata) : NULL,
                                      flags, setup_func, setup_data, ret_pid,
                                      ret_stdin, ret_stdout, ret_stderr, error);
        g_strfreev (argv);
                         ...
                         GError ** error, ...)
{
    ...
    switch (desktop_file->type)
    {
        case EGG_DESKTOP_FILE_TYPE_APPLICATION:
            va_start (args, error);
            success =
                egg_desktop_file_launchv (desktop_file, documents, args, error);
            va_end (args);
            break;
            ...
            if (documents)
        {
            ...
            va_start (args, error);
            success =
                egg_desktop_file_launchv (app_desktop_file, documents, args,
                                          error);
            va_end (args);
...
egg_set_desktop_file (const char *desktop_file_path)
{
    ...
    if (error)
    {
        g_warning ("Could not load desktop file '%s': %s", desktop_file_path,
                   error->message);
        g_error_free (error);
    }
