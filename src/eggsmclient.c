...
static gboolean
sm_client_post_parse_func (GOptionContext * context, GOptionGroup * group,
...
egg_sm_client_get_option_group (void)
{
    ...
    group =
        g_option_group_new ("sm-client", _("Session management options:"),
                            _("Show session management options"), NULL, NULL);
                                   ...
                                   const char **argv)
{
    ...
    if (EGG_SM_CLIENT_GET_CLASS (client)->set_restart_command)
        EGG_SM_CLIENT_GET_CLASS (client)->set_restart_command (client, argc,
                                                               argv);
}
...
egg_sm_client_quit_requested (EggSMClient * client)
{
    ...
    if (!g_signal_has_handler_pending
        (client, signals[QUIT_REQUESTED], 0, FALSE))
    {
...
static void
egg_sm_client_debug_handler (const char *log_domain, GLogLevelFlags log_level,
