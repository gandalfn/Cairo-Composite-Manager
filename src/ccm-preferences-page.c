...
static void ccm_preferences_page_iface_init (CCMPreferencesPagePluginClass *
                                             iface);
                                            ...
                                            CCMConfig * config)
{
    ...
    filename =
        ccm_config_get_string (self->priv->
                               screen_options[CCM_SCREEN_BACKGROUND], NULL);
        ...
        && !g_file_test (filename, G_FILE_TEST_IS_DIR))
    {
        ...
        if (pixbuf)
        {
            ...
            {
                height =
                    (int) ((float) gdk_pixbuf_get_height (pixbuf) *
                           ((float) width /
            ...
            {
                width =
                    (int) ((float) gdk_pixbuf_get_width (pixbuf) *
                           ((float) height /
                                                     ...
                                                     CCMConfig * config)
{
    ...
    widget =
        GTK_WIDGET (gtk_builder_get_object
                    (self->priv->builder, "color_background"));
    gtk_widget_set_sensitive (widget, !use_root_background);

    widget =
        GTK_WIDGET (gtk_builder_get_object
                    (self->priv->builder, "background_button"));
    gtk_widget_set_sensitive (widget, !use_root_background);

    widget =
        GTK_WIDGET (gtk_builder_get_object
                    (self->priv->builder, "background_x"));
    gtk_widget_set_sensitive (widget, !use_root_background);

    widget =
        GTK_WIDGET (gtk_builder_get_object
                    (self->priv->builder, "background_y"));
    gtk_widget_set_sensitive (widget, !use_root_background);
    ...
    for (cpt = 0; cpt < CCM_SCREEN_OPTION_N; ++cpt)
    {
        ...
        if (cpt == CCM_SCREEN_USE_ROOT_BACKGROUND)
        {
            ...
            g_signal_connect_swapped (self->priv->screen_options[cpt],
                                      "changed",
                                      G_CALLBACK
                                      (ccm_preferences_page_on_use_root_background_changed),
                                      self);
        }
        if (cpt == CCM_SCREEN_BACKGROUND)
        {
            ...
            g_signal_connect_swapped (self->priv->screen_options[cpt],
                                      "changed",
                                      G_CALLBACK
                                      (ccm_preferences_page_on_background_changed),
                                      self);
        }
    }
}
                                         ...
                                         GtkTreeSelection * selection)
{
    ...
    sections =
        GTK_NOTEBOOK (gtk_builder_get_object (self->priv->builder, "sections"));
    if (!sections)
        return;
...
ccm_preferences_page_init_sections_list (CCMPreferencesPage * self)
{
    ...
    sections_view =
        GTK_TREE_VIEW (gtk_builder_get_object
                       (self->priv->builder, "sections_view"));
    if (!sections_view)
        return;
    ...
    g_signal_connect_swapped (selection, "changed",
                              G_CALLBACK
                              (ccm_preferences_page_on_section_changed), self);
}
                                       ...
                                       CCMCellRendererExtension * cell)
{
    ...
    plugins_list =
        GTK_TREE_MODEL (gtk_builder_get_object
                        (self->priv->builder, "plugins_list"));
    if (!plugins_list)
        return;
    ...
    plugins_view =
        GTK_TREE_VIEW (gtk_builder_get_object
                       (self->priv->builder, "plugins_view"));
    if (!plugins_view)
        return;
    ...
    plugins_column =
        GTK_TREE_VIEW_COLUMN (gtk_builder_get_object
                              (self->priv->builder, "plugins_column"));
    if (!plugins_column)
        return;
                                          ...
                                          GtkTreeModel * plugins_list)
{
    ...
    plugins =
        ccm_config_get_string_list (self->priv->
                                    screen_options[CCM_SCREEN_PLUGINS], NULL);
    if (plugins)
    {
        ...
        if (name)
        {
            ...
            if (active && !found)
            {
                ...
                update = TRUE;
            if (update)
                ccm_config_set_string_list (self->priv->
                                            screen_options[CCM_SCREEN_PLUGINS],
                                            new, NULL);
            if (new)
            {
            ...
            g_signal_emit (self, signals[PLUGIN_STATE_CHANGED], 0, name,
                           active);
            g_free (name);
        }
...
ccm_preferences_page_init_plugins_list (CCMPreferencesPage * self)
{
    ...
    g_signal_connect_swapped (selection, "changed",
                              G_CALLBACK
                              (ccm_preferences_page_on_plugin_changed), self);
    ...
    gtk_tree_view_column_add_attribute (plugins_column,
                                        GTK_CELL_RENDERER (cell), "name", 0);
    gtk_tree_view_column_add_attribute (plugins_column,
                                        GTK_CELL_RENDERER (cell), "description",
                                        1);
    gtk_tree_view_column_add_attribute (plugins_column,
                                        GTK_CELL_RENDERER (cell), "version", 2);
    gtk_tree_view_column_add_attribute (plugins_column,
                                        GTK_CELL_RENDERER (cell), "enabled", 3);
    ...
    plugins =
        ccm_extension_loader_get_screen_window_plugins (self->priv->
                                                        plugin_loader);
...
ccm_preferences_page_init_backends_list (CCMPreferencesPage * self)
{
    ...
    gtk_list_store_set (backends, &iter, 0, "openvg", 1, "OpenVG rendering",
                        -1);
#endif
    ...
    gtk_list_store_prepend (backends, &iter);
    gtk_list_store_set (backends, &iter, 0, "glitz-tfp", 1,
                        "OpenGL TFP rendering", -1);
#endif
                                               ...
                                               int response, GtkDialog * dialog)
{
    if (response == GTK_RESPONSE_ACCEPT)
    {
        ...
        ccm_config_set_string (self->priv->
                               screen_options[CCM_SCREEN_BACKGROUND], filename,
                               NULL);
        g_free (filename);
    }
                                                ...
                                                GtkWidget * widget)
{
    ...
    button =
        GTK_TOGGLE_BUTTON (gtk_builder_get_object
                           (self->priv->builder, "enable_composite_desktop"));
    if (!button)
        return;
    ...
    g_signal_connect_swapped (button, "toggled",
                              G_CALLBACK
                              (ccm_preferences_page_on_composite_desktop_toggled),
                              self);

    ccm_preferences_page_init_backends_list (self);

    self->priv->backend =
        ccm_config_get_string (self->priv->screen_options[CCM_SCREEN_BACKEND],
                               NULL);
    if (self->priv->backend)
    {
        GtkComboBox *combo;
        ...
        native =
            ccm_config_get_boolean (self->priv->
                                    screen_options[CCM_SCREEN_PIXMAP], NULL);
    ...
    refresh_rate =
        ccm_config_get_integer (self->priv->
                                screen_options[CCM_SCREEN_REFRESH_RATE], NULL);
    spin =
        GTK_SPIN_BUTTON (gtk_builder_get_object
                         (self->priv->builder, "refresh_rate"));
    if (!spin)
        return;
    ...
    g_signal_connect_swapped (spin, "value-changed",
                              G_CALLBACK
                              (ccm_preferences_page_on_refresh_rate_changed),
                              self);
                                                ...
                                                GtkWidget * desktop_section)
{
    ...
    if (widget)
    {
        ...
        g_signal_connect_swapped (G_OBJECT (button_background), "clicked",
                                  G_CALLBACK
                                  (ccm_preferences_page_on_button_background_clicked),
                                  self);
        ccm_preferences_page_on_background_changed (self,
                                                    self->priv->
                                                    screen_options
...
ccm_preferences_page_init_general_section (CCMPreferencesPage * self)
{
    ...
    general_section =
        GTK_WIDGET (gtk_builder_get_object
                    (self->priv->builder, "general_section"));
    if (!general_section)
        return;
...
ccm_preferences_page_init_desktop_section (CCMPreferencesPage * self)
{
    ...
    desktop_section =
        GTK_WIDGET (gtk_builder_get_object
                    (self->priv->builder, "desktop_section"));
    if (!desktop_section)
        return;
...
ccm_preferences_page_init_windows_section (CCMPreferencesPage * self)
{
    ...
    windows_section =
        GTK_WIDGET (gtk_builder_get_object
                    (self->priv->builder, "windows_section"));
    if (!windows_section)
        return;
...
ccm_preferences_page_init_effects_section (CCMPreferencesPage * self)
{
    ...
    effects_section =
        GTK_WIDGET (gtk_builder_get_object
                    (self->priv->builder, "effects_section"));
    if (!effects_section)
        return;
...
ccm_preferences_page_init_accessibility_section (CCMPreferencesPage * self)
{
    ...
    accessibility_section =
        GTK_WIDGET (gtk_builder_get_object
                    (self->priv->builder, "accessibility_section"));
...
ccm_preferences_page_init_utilities_section (CCMPreferencesPage * self)
{
    ...
    utilities_section =
        GTK_WIDGET (gtk_builder_get_object
                    (self->priv->builder, "utilities_section"));
...
ccm_preferences_page_get_plugins (CCMPreferencesPage * self)
{
    ...
    plugins =
        ccm_extension_loader_get_preferences_plugins (self->priv->
                                                      plugin_loader);
    for (item = plugins; item; item = item->next)
    {
        ...
        CCMPreferencesPagePlugin *plugin =
            g_object_new (type, "parent", prev, NULL);
