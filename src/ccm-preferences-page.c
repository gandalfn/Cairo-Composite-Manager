...
G_DEFINE_TYPE_EXTENDED (CCMPreferencesPage, ccm_preferences_page, G_TYPE_OBJECT, 0,
                        G_IMPLEMENT_INTERFACE (CCM_TYPE_PREFERENCES_PAGE_PLUGIN,
...
ccm_preferences_page_class_init (CCMPreferencesPageClass * klass)
{
    ...
    signals[NEED_RESTART] =
        g_signal_new ("need-restart", G_OBJECT_CLASS_TYPE (object_class),
                      G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                      ccm_cclosure_marshal_VOID__POINTER_POINTER, G_TYPE_NONE,
                      2, G_TYPE_POINTER, G_TYPE_POINTER);

    signals[PLUGIN_STATE_CHANGED] =
        g_signal_new ("plugin-state-changed",
                      G_OBJECT_CLASS_TYPE (object_class), G_SIGNAL_RUN_LAST, 0,
                      NULL, NULL, ccm_cclosure_marshal_VOID__STRING_BOOLEAN,
...
{
    iface->init_general_section =
        impl_ccm_preferences_page_init_general_section;
    iface->init_desktop_section =
        impl_ccm_preferences_page_init_desktop_section;
}
                                            ...
                                            CCMConfig * config)
{
        ...
        !g_file_test (filename, G_FILE_TEST_IS_DIR))
    {
        ...
        if (pixbuf)
        {
            ...
            image =  GTK_IMAGE (gtk_builder_get_object
                                (self->priv->builder, "background_image"));
            if (!image)
            {
...
ccm_preferences_page_load_config (CCMPreferencesPage * self)
{
    ...
    for (cpt = 0; cpt < CCM_SCREEN_OPTION_N; ++cpt)
    {
        self->priv->screen_options[cpt] =
            ccm_config_new (self->priv->screen_num, NULL,
...
static void
ccm_preferences_page_on_plugin_edited (CCMPreferencesPage * self, gchar * path,
                                       gchar * text,
                                          ...
                                          GtkTreeModel * plugins_list)
{
    ...
    plugins = ccm_config_get_string_list (self->priv->screen_options[CCM_SCREEN_PLUGINS], NULL);
    if (plugins)
    {
        ...
        if (name)
        {
            for (item = plugins; item; item = item->next)
            {
                ...
                {
                    new = g_slist_prepend (new, g_ascii_strdown (item->data, strlen (item->data)));
                }
            ...
            if (active && !found)
            {
                ...
                update = TRUE;
            if (update)
                ccm_config_set_string_list (self->priv-> screen_options[CCM_SCREEN_PLUGINS],
                                            new, NULL);
            if (new)
            {
...
ccm_preferences_page_init_plugins_list (CCMPreferencesPage * self)
{
    ...
    plugins_view = GTK_TREE_VIEW (gtk_builder_get_object (self->priv->builder, "plugins_view"));
    if (!plugins_view)
        return;
    ...
    plugins = ccm_extension_loader_get_screen_window_plugins (self->priv->plugin_loader);

    actives = ccm_config_get_string_list (self->priv->screen_options[CCM_SCREEN_PLUGINS], NULL);
    for (item = plugins; item; item = item->next)
    {
        ...
        for (item_active = actives; item_active && !active; item_active = item_active->next)
            active |= !g_ascii_strcasecmp (name, item_active->data);

        gtk_list_store_append (plugins_list, &iter);
        gtk_list_store_set (plugins_list, &iter, 0, name ? name : "", 1,
                            description ? description : "", 2,
                            version ? version : "", 3, active, -1);
        g_signal_emit (self, signals[PLUGIN_STATE_CHANGED], 0, name, active);
    }
    if (actives)
    {
    ...
    g_signal_connect_swapped (plugins_list, "row-changed",
                              G_CALLBACK
                              (ccm_preferences_page_on_plugins_modified), self);
}
...
ccm_preferences_page_init_backends_list (CCMPreferencesPage * self)
{
    ...
    backends =
        GTK_LIST_STORE (gtk_builder_get_object
                        (self->priv->builder, "backends_list"));
    if (!backends)
        return;
                                                   ...
                                                   GtkToggleButton * button)
{
    ...
    unmanaged =
        ccm_config_get_integer_list (self->priv->
                                     general_options
                                     [CCM_GENERAL_UNMANAGED_SCREEN], NULL);
    ...
    if (gtk_toggle_button_get_active (button))
    {
        ...
        ccm_config_set_integer_list (self->priv->
                                     general_options
                                     [CCM_GENERAL_UNMANAGED_SCREEN], list,
                                     NULL);
        g_slist_free (list);
    }
    else
    {
        unmanaged =
            g_slist_prepend (unmanaged,
                             GINT_TO_POINTER (self->priv->screen_num));
        ccm_config_set_integer_list (self->priv->
                                     general_options
                                     [CCM_GENERAL_UNMANAGED_SCREEN], unmanaged,
                                     NULL);
        g_slist_free (unmanaged);
    }
...
ccm_preferences_page_change_backend (CCMPreferencesPage * self, gchar * name)
{
    ...
    if (!g_ascii_strcasecmp (name, "image"))
    {
        ccm_config_set_boolean (self->priv->screen_options[CCM_SCREEN_PIXMAP],
                                FALSE, NULL);
        ccm_config_set_boolean (self->priv->
                                screen_options[CCM_SCREEN_USE_BUFFERED], TRUE,
                                NULL);
                                                   ...
                                                   GtkButton * button)
{
    ...
    g_signal_connect_swapped (G_OBJECT (dialog), "response",
                              G_CALLBACK
                              (ccm_preferences_page_on_file_chooser_response),
                              self);
}

static void
impl_ccm_preferences_page_init_general_section (CCMPreferencesPagePlugin *
                                                plugin,
                                                CCMPreferencesPage * self,
                                                GtkWidget * widget)
{
    ...
    unmanaged =
        ccm_config_get_integer_list (self->priv->general_options[CCM_GENERAL_UNMANAGED_SCREEN], NULL);
    ...
    self->priv->backend = ccm_config_get_string (self->priv->screen_options[CCM_SCREEN_BACKEND], NULL);
    if (self->priv->backend)
    {
        GtkComboBox *combo;
        GtkTreeModel *backends;
        ...
        combo = GTK_COMBO_BOX (gtk_builder_get_object (self->priv->builder, "backend"));
        if (!combo)
            return;
        ...
        backends = GTK_TREE_MODEL (gtk_builder_get_object (self->priv->builder, "backends_list"));
        if (!backends)
            return;
        native = ccm_config_get_boolean (self->priv->screen_options[CCM_SCREEN_PIXMAP], NULL);
...
static void
impl_ccm_preferences_page_init_desktop_section (CCMPreferencesPagePlugin *
                                                plugin,
                                                CCMPreferencesPage * self,
                                                GtkWidget * desktop_section)
{
    GtkWidget *widget = GTK_WIDGET (gtk_builder_get_object (self->priv->builder, "background"));
    if (widget)
    {
        gboolean use_root = ccm_config_get_boolean (self->priv->screen_options[CCM_SCREEN_USE_ROOT_BACKGROUND],
                                                    NULL);
        gint screen_num = ccm_preferences_page_get_screen_num (self);

        gtk_box_pack_start (GTK_BOX (desktop_section), widget, FALSE, TRUE, 0);
        ...
        GtkButton *button_background =
            GTK_BUTTON (gtk_builder_get_object (self->priv->builder,
                                                "background_button"));
        g_signal_connect_swapped (G_OBJECT (button_background), "clicked",
        ...
        CCMConfigColorButton *color_background =
            CCM_CONFIG_COLOR_BUTTON (gtk_builder_get_object (self->priv->builder, "color_background"));
        g_object_set (color_background, "screen", screen_num, NULL);
        gtk_widget_set_sensitive (GTK_WIDGET (color_background), !use_root);

        widget = GTK_WIDGET (gtk_builder_get_object (self->priv->builder, "background_button"));
        ...
        widget = GTK_WIDGET (gtk_builder_get_object (self->priv->builder, "background_x"));
        gtk_widget_set_sensitive (GTK_WIDGET (widget), !use_root);
        ...
        widget = GTK_WIDGET (gtk_builder_get_object (self->priv->builder, "background_y"));
        gtk_widget_set_sensitive (GTK_WIDGET (widget), !use_root);
    }
...
ccm_preferences_page_init_general_section (CCMPreferencesPage * self)
{
    ...
    general_section = GTK_WIDGET (gtk_builder_get_object (self->priv->builder, "general_section"));
    if (!general_section)
        return;
...
ccm_preferences_page_init_desktop_section (CCMPreferencesPage * self)
{
    ...
    desktop_section = GTK_WIDGET (gtk_builder_get_object (self->priv->builder, "desktop_section"));
    if (!desktop_section)
        return;
...
ccm_preferences_page_init_windows_section (CCMPreferencesPage * self)
{
    ...
    windows_section = GTK_WIDGET (gtk_builder_get_object (self->priv->builder, "windows_section"));
    if (!windows_section)
        return;
...
ccm_preferences_page_init_effects_section (CCMPreferencesPage * self)
{
    ...
    effects_section = GTK_WIDGET (gtk_builder_get_object (self->priv->builder, "effects_section"));
    if (!effects_section)
        return;
...
ccm_preferences_page_init_accessibility_section (CCMPreferencesPage * self)
{
    ...
    accessibility_section = GTK_WIDGET (gtk_builder_get_object (self->priv->builder, "accessibility_section"));
...
ccm_preferences_page_init_utilities_section (CCMPreferencesPage * self)
{
    ...
    utilities_section = GTK_WIDGET (gtk_builder_get_object (self->priv->builder, "utilities_section"));
    if (!utilities_section)
        return;

    ccm_preferences_page_plugin_init_utilities_section (self->priv->plugin,
                                                        self,
                                                        utilities_section);
}
...
ccm_preferences_page_get_plugins (CCMPreferencesPage * self)
{
    ...
    plugins = ccm_extension_loader_get_preferences_plugins (self->priv->plugin_loader);
    for (item = plugins; item; item = item->next)
    {
...
ccm_preferences_page_new (gint screen_num)
{
    ...
    gtk_builder_add_from_file (self->priv->builder, UI_DIR "/ccm-preferences.ui", NULL);
                                          ...
                                          CCMPreferencesPageSection section)
{
    ...
    sections_view = GTK_TREE_VIEW (gtk_builder_get_object (self->priv->builder, "sections_view"));
    if (!sections_view)
        return;
                                ...
                                CCMPreferencesPageSection section)
{
    ...
    if (gtk_tree_model_get_iter_first (sections_list, &iter))
    {
        do
        {
            ...
            gtk_tree_model_get (sections_list, &iter, 0, &page, 3, &active, 4,
                                &count, -1);
            if (section == page)
            {
                count++;
                active = count > 0;
                gtk_list_store_set (GTK_LIST_STORE (sections_list), &iter, 3,
                                    active, 4, count, -1);
            }
        }
        while (gtk_tree_model_iter_next (sections_list, &iter));
                                ...
                                CCMPreferencesPageSection section)
{
    ...
    sections_list =
        GTK_TREE_MODEL (gtk_builder_get_object
                        (self->priv->builder, "sections_list"));
    if (!sections_list)
        return;
                                                ...
                                                GtkWidget * widget)
{
    ...
    gboolean wactive =
        (gboolean) GPOINTER_TO_INT (g_object_get_qdata (G_OBJECT (widget),
                                                        CCM_WIDGET_PLUGIN_ACTIVE));
    CCMPreferencesPageSection section =
        (CCMPreferencesPageSection) GPOINTER_TO_INT (g_object_get_qdata (G_OBJECT (widget),
                                                                         CCM_WIDGET_SECTION));
                                              ...
                                              gchar * plugin)
{
    ...
    g_signal_connect (G_OBJECT (self), "plugin-state-changed",
                      G_CALLBACK
                      (ccm_preferences_page_section_on_plugin_changed), widget);
}
