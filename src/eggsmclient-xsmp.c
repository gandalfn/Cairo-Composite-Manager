...
static void sm_client_xsmp_set_restart_command (EggSMClient * client, int argc,
                                                const char **argv);
static void sm_client_xsmp_will_quit (EggSMClient * client, gboolean will_quit);
static gboolean sm_client_xsmp_end_session (EggSMClient * client,
                                            EggSMClientEndStyle style,
                                            gboolean request_confirmation);

static void xsmp_save_yourself (SmcConn smc_conn, SmPointer client_data,
...
static void xsmp_interact (SmcConn smc_conn, SmPointer client_data);

static SmProp *array_prop (const char *name, ...);
static SmProp *ptrarray_prop (const char *name, GPtrArray * values);
static SmProp *string_prop (const char *name, const char *value);
static SmProp *card8_prop (const char *name, unsigned char value);
...
static void smc_error_handler (SmcConn smc_conn, Bool swap,
                               int offending_minor_opcode,
                               unsigned long offending_sequence,
                               int error_class, int severity, SmPointer values);
...
sm_client_xsmp_set_initial_properties (gpointer user_data)
{
    ...
    if (desktop_file)
    {
        ...
        {
            if (egg_desktop_file_get_boolean
                (desktop_file, "X-GNOME-AutoRestart", NULL))
        ...
        if (!xsmp->set_restart_command)
        {
            ...
            {
                egg_sm_client_set_restart_command (EGG_SM_CLIENT (xsmp), argc,
                                                   (const char **) argv);
                g_strfreev (argv);
            }
            else
    ...
    set_properties (xsmp, string_prop (SmProgram, g_get_prgname ()),
                    ptrarray_prop (SmCloneCommand, clone),
                    ptrarray_prop (SmRestartCommand, restart),
                    string_prop (SmUserID, g_get_user_name ()),
                    string_prop (SmProcessID, pid_str),
                    card8_prop (SmRestartStyleHint, xsmp->restart_style), NULL);
...
static void
sm_client_xsmp_startup (EggSMClient * client, const char *client_id)
{
...
static void
sm_client_xsmp_set_restart_command (EggSMClient * client, int argc,
                                    const char **argv)
{
    EggSMClientXSMP *xsmp = (EggSMClientXSMP *) client;
    int i;
...
static void
sm_client_xsmp_will_quit (EggSMClient * client, gboolean will_quit)
{
    EggSMClientXSMP *xsmp = (EggSMClientXSMP *) client;
...
static gboolean
sm_client_xsmp_end_session (EggSMClient * client, EggSMClientEndStyle style,
                            gboolean request_confirmation)
{
    EggSMClientXSMP *xsmp = (EggSMClientXSMP *) client;
    int save_type;
    ...
    g_debug
        ("Sending SaveYourselfRequest(SmSaveGlobal, Shutdown, SmInteractStyleAny, %sFast)",
         request_confirmation ? "!" : "");
    SmcRequestSaveYourself (xsmp->connection, save_type, True,  /* shutdown */
                            SmInteractStyleAny, !request_confirmation,  /* fast */
                            True /* global */ );
    return TRUE;
}
...
{
    gboolean want_idle = xsmp->waiting_to_emit_quit
        || xsmp->waiting_to_emit_quit_cancelled || xsmp->waiting_to_save_myself;
...
fix_broken_state (EggSMClientXSMP * xsmp, const char *message,
                  gboolean send_interact_done, gboolean send_save_yourself_done)
{
    ...
    xsmp->state =
        send_save_yourself_done ? XSMP_STATE_SAVE_YOURSELF_DONE :
        XSMP_STATE_IDLE;
}
...
static void
xsmp_save_yourself (SmcConn smc_conn, SmPointer client_data, int save_type,
                    Bool shutdown, int interact_style, Bool fast)
{
    ...
    g_debug ("Received SaveYourself(%s, %s, %s, %s) in state %s",
             save_type == SmSaveLocal ? "SmSaveLocal" : save_type ==
             SmSaveGlobal ? "SmSaveGlobal" : "SmSaveBoth",
    ...
    if (xsmp->expecting_initial_save_yourself)
    {
        ...
        if (save_type == SmSaveLocal && interact_style == SmInteractStyleNone
            && !shutdown && !fast)
        {
    ...
    wants_quit_requested =
        g_signal_has_handler_pending (xsmp,
                                      g_signal_lookup ("quit_requested",
                                                       EGG_TYPE_SM_CLIENT), 0,
                                      FALSE);
...
do_save_yourself (EggSMClientXSMP * xsmp)
{
    ...
    if (xsmp->need_quit_requested)
    {
        ...
        SmcInteractRequest (xsmp->connection,
                            xsmp->
                            interact_errors ? SmDialogError : SmDialogNormal,
                            xsmp_interact, xsmp);
        return;
    }
...
save_state (EggSMClientXSMP * xsmp)
{
    ...
    {
        restart =
            generate_command (xsmp->restart_command, xsmp->client_id, NULL);
        set_properties (xsmp, ptrarray_prop (SmRestartCommand, restart), NULL);
        g_ptr_array_free (restart, TRUE);
    ...
    if (desktop_file)
    {
                                          ...
                                          G_KEY_FILE_KEEP_TRANSLATIONS, NULL))
        {
            ...
            for (g = 0; groups[g]; g++)
            {
                ...
                {
                    value =
                        g_key_file_get_value (state_file, groups[g], keys[k],
                                              NULL);
                    if (value)
                    {
                        g_key_file_set_value (merged_file, groups[g], keys[k],
                                              value);
                        g_free (value);
                    }
                }
            ...
            restart = generate_command (xsmp->restart_command, NULL, "%k");
            ...
            g_key_file_set_string (state_file, EGG_DESKTOP_FILE_GROUP,
                                   EGG_DESKTOP_FILE_KEY_EXEC, exec);
            g_free (exec);
        }
        else
            desktop_file = NULL;
    ...
    while (1)
    {
        state_file_path =
            g_strdup_printf ("%s%csession-state%c%s-%ld.%s",
                             g_get_user_config_dir (), G_DIR_SEPARATOR,
                             G_DIR_SEPARATOR, g_get_prgname (),
        ...
        if (fd == -1)
        {
            ...
            g_warning ("Could not create file '%s': %s", state_file_path,
                       g_strerror (errno));
            g_free (state_file_path);
            state_file_path = NULL;
            break;
    ...
    restart =
        generate_command (xsmp->restart_command, xsmp->client_id,
                          state_file_path);
    set_properties (xsmp, ptrarray_prop (SmRestartCommand, restart), NULL);
    g_ptr_array_free (restart, TRUE);
    ...
    {
        set_properties (xsmp,
                        array_prop (SmDiscardCommand, "/bin/rm", "-rf",
                                    state_file_path, NULL), NULL);
        g_free (state_file_path);
    }
}

static void
xsmp_interact (SmcConn smc_conn, SmPointer client_data)
{
    EggSMClientXSMP *xsmp = client_data;
...
static void ice_error_handler (IceConn ice_conn, Bool swap,
                               int offending_minor_opcode,
                               unsigned long offending_sequence,
                               int error_class, int severity,
                               IcePointer values);
static void ice_io_error_handler (IceConn ice_conn);
static void ice_connection_watch (IceConn ice_conn, IcePointer client_data,
...
static void
ice_connection_watch (IceConn ice_conn, IcePointer client_data, Bool opening,
                      IcePointer * watch_data)
{
    ...
    if (opening)
    {
        ...
        watch_id =
            g_io_add_watch (channel, G_IO_IN | G_IO_ERR, ice_iochannel_watch,
                            ice_conn);
...
static void
ice_error_handler (IceConn ice_conn, Bool swap, int offending_minor_opcode,
...
static void
smc_error_handler (SmcConn smc_conn, Bool swap, int offending_minor_opcode,
                   unsigned long offending_sequence, int error_class,
