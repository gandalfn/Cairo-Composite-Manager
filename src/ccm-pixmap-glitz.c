...
ccm_pixmap_glitz_create_gl_drawable (CCMPixmapGlitz * self)
{
    ...
    if (!self->priv->gl_pixmap)
    {
        ...
        {
            format =
                glitz_glx_find_drawable_format_for_visual (CCM_DISPLAY_XDISPLAY
                                                           (display),
                                                           CCM_SCREEN_NUMBER
                                                           (screen),
                                                           XVisualIDFromVisual
        ...
        self->priv->gl_pixmap =
            glitz_glx_create_drawable_for_pixmap (CCM_DISPLAY_XDISPLAY
                                                  (display),
        ...
        self->priv->gl_format =
            glitz_find_format (self->priv->gl_pixmap,
...
ccm_pixmap_glitz_bind (CCMPixmap * pixmap)
{
    ...
    if (!self->priv->gl_surface && ccm_pixmap_glitz_create_gl_drawable (self))
    {
        cairo_rectangle_t geometry;
        ...
        self->priv->gl_surface =
            glitz_surface_create (self->priv->gl_pixmap, self->priv->gl_format,
                                  geometry.width, geometry.height, 0, NULL);
    }
}
...
ccm_pixmap_glitz_repair (CCMDrawable * drawable, CCMRegion * area)
{
    ...
    {
        ret =
            glitz_surface_bind_tex_image (self->priv->gl_surface,
                                          self->priv->gl_pixmap);
    }
...
ccm_pixmap_glitz_get_surface (CCMDrawable * drawable)
{
    ...
    if (!self->priv->surface)
    {
        if (self->priv->gl_surface)
            self->priv->surface =
                cairo_glitz_surface_create (self->priv->gl_surface);
    }
