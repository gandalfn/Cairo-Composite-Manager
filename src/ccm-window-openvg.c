...
ccm_window_openvg_get_visual_info (CCMWindowOpenVG * self)
{
    ...
    visinfo =
        glXChooseVisual (CCM_DISPLAY_XDISPLAY (display), screen->number,
                         attribs);
...
ccm_window_openvg_get_gl_context (CCMWindowOpenVG * self)
{
    ...
    if (!context)
    {
        ...
        context =
            glXCreateContext (CCM_DISPLAY_XDISPLAY (display), visinfo, NULL,
                              GL_TRUE);
...
ccm_window_openvg_get_surface (CCMDrawable * drawable)
{
    ...
    surface =
        cairo_openvg_surface_create ((int) geometry.width,
                                     (int) geometry.height);
...
typedef void (*glXCopySubBufferMESAProc) (Display *, Drawable, int, int, int,
                                          int);
...
ccm_window_openvg_vsync (CCMWindowOpenVG * self, gint interval)
{
    ...
    if (supported && initialized != interval)
    {
        ...
        s = (glXSwapIntervalSGIProc) glXGetProcAddress ((const guchar *)
                                                        "glXSwapIntervalSGI");
        if (s)
        {
            s (interval);
        ...
        m = (glXSwapIntervalMESAProc) glXGetProcAddress ((const guchar *)
                                                         "glXSwapIntervalMESA");
        if (m)
        {
            m (interval);
...
ccm_window_openvg_flush_region (CCMDrawable * drawable, CCMRegion * region)
{
    ...
    {
        csb =
            (glXCopySubBufferMESAProc) glXGetProcAddress ((const guchar *)
    ...
    if (csb)
    {
        ...
        for (cpt = 0; cpt < nb_rects; ++cpt)
        {
            ...
            gint width =
                rects[cpt].width + x >
                screen->xscreen->width ? screen->xscreen->width -
                x : rects[cpt].width;
            gint height =
                rects[cpt].height + y >
                screen->xscreen->height ? screen->xscreen->height -
                y : rects[cpt].height;
