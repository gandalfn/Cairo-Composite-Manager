...
ccm_window_glitz_finalize (GObject * object)
{
    ...
    if (self->priv->gl_drawable)
        glitz_drawable_destroy (self->priv->gl_drawable);
...
ccm_window_glitz_create_gl_drawable (CCMWindowGlitz * self)
{
    ...
    if (!self->priv->gl_drawable)
    {
        ...
        gboolean indirect;
        unsigned long mask =
            GLITZ_FORMAT_DOUBLEBUFFER_MASK | GLITZ_FORMAT_RED_SIZE_MASK |
        ...
        format =
            glitz_glx_find_window_format (CCM_DISPLAY_XDISPLAY (display),
        ...
        glitz_glx_set_render_type (CCM_DISPLAY_XDISPLAY (display),
                                   CCM_SCREEN_NUMBER (screen), !indirect);
#endif

        g_object_set (self, "mask_y_invert",
                      format->scanline_order ==
                      GLITZ_PIXEL_SCANLINE_ORDER_BOTTOM_UP, NULL);

        self->priv->gl_drawable =
            glitz_glx_create_drawable_for_window (CCM_DISPLAY_XDISPLAY
                                                  (display),
        ...
        self->priv->gl_format =
            glitz_find_format (self->priv->gl_drawable,
                               GLITZ_FORMAT_RED_SIZE_MASK |
                               GLITZ_FORMAT_GREEN_SIZE_MASK |
                               GLITZ_FORMAT_BLUE_SIZE_MASK |
                               GLITZ_FORMAT_ALPHA_SIZE_MASK |
                               GLITZ_FORMAT_FOURCC_MASK, &templ, 0);
...
ccm_window_glitz_create_gl_surface (CCMWindowGlitz * self)
{
    ...
    if (!self->priv->gl_surface && ccm_window_glitz_create_gl_drawable (self))
    {
        cairo_rectangle_t geometry;
        ...
        self->priv->gl_surface =
            glitz_surface_create (self->priv->gl_drawable,
                                  self->priv->gl_format, geometry.width,
                                  geometry.height, 0, NULL);
        if (self->priv->gl_surface)
        {
...
static CCMPixmap *
ccm_window_glitz_create_pixmap (CCMWindow * self, int width, int height,
                                int depth)
{
    ...
    format =
        glitz_glx_find_window_format (CCM_DISPLAY_XDISPLAY (display),
                                      CCM_SCREEN_NUMBER (screen), mask, &templ,
                                      0);
