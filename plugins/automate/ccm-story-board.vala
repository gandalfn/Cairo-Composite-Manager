...
namespace CCM
{
    class StoryBoard : GLib.Object
    {
        ...
        public StoryBoard (CCM.Screen screen, string name,
                           List<weak CCM.Window> ignore)
        {
        ...
        private void on_event (CCM.Display display, X.Event event)
        {
            ...
            {
                CCM.Action action =
                    actions.last () != null ? actions.last ().data : null;
                bool insert = true;
                ...
                if (hint_motion && action != null)
                {
                    if (action is CCM.ActionPointerMotion)
                    {
                        ...
                        ((CCM.ActionPointerMotion) action).time =
                            (int) ((double) diff / (double) 1000);
                        insert = false;
                    }
                }
            ...
            else if (event.type == X.EventType.ButtonPress)
            {
                ...
                {
                    var action =
                        new CCM.ActionPointerPress (screen, event, diff,
                                                    ignore);
                    actions.append (action);
                }
            ...
            else if (event.type == X.EventType.ButtonRelease)
            {
                ...
                {
                    var action =
                        new CCM.ActionPointerRelease (screen, event, diff,
                                                      ignore);
                    actions.append (action);
                }
                catch (CCM.ActionError ex)
        ...
        public string to_string (string format = "%ccmsb")
        {
