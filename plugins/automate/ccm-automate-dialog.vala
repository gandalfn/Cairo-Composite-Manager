...
namespace CCM
{
    ...
    {
        private const string UI_FILE = "/usr/share/cairo-compmgr/ui/ccm-automate.ui";
		...
		private void
		on_new_frame(CCM.Timeline timeline, int num_frame)
		{
			int width, height;
			int x, y;
		...
		private void
		on_completed(CCM.Timeline timeline)
		{
			string stock;
		...
		private bool
		on_expose_event(Gtk.Widget widget, Gdk.EventExpose event)
		{
			...
			ctx.set_source_rgba((double)widget.style.bg[Gtk.StateType.NORMAL].red / 65535,
								(double)widget.style.bg[Gtk.StateType.NORMAL].green / 65535,
								(double)widget.style.bg[Gtk.StateType.NORMAL].blue / 65535,
								0.85);
			((Cairo.CCMContext)ctx).notebook_page_round(0, 0, width, height, 
									0, close.allocation.width, 
									close.allocation.height, 6);
			...
			ctx.set_source_rgba((double)widget.style.bg[Gtk.StateType.SELECTED].red / 65535,
		...
		private bool
		on_close(Gtk.Widget widget, Gdk.EventButton event)
		{
			if (timeline.get_direction() == CCM.TimelineDirection.FORWARD)
		...
		private void
		on_record_clicked(Gtk.Button button)
		{
		...
		private void
		on_stop_clicked(Gtk.Button button)
		{
			if (story_board != null)
		...
		construct_ui()
		{
			try 
			{
				...
				// Get record button press
				hint_motion = builder.get_object ("hint_motion") as Gtk.CheckButton;
			}
			catch (GLib.Error ex)
			{
