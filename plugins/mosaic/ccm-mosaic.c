...
ccm_mosaic_find_area (CCMMosaic * self, CCMWindow * window)
{
    ...
    for (cpt = 0; cpt < self->priv->nb_areas; ++cpt)
    {
        ...
        scale =
            MIN (self->priv->areas[cpt].geometry.height / win_area->height,
                 self->priv->areas[cpt].geometry.width / win_area->width);

        if (sqrt (pow (1.0f - scale, 2)) < search
...
ccm_mosaic_set_window_transform (CCMMosaic * self)
{
    ...
    for (cpt = 0; cpt < self->priv->nb_areas; ++cpt)
    {
        ...
        CCMMosaic *plugin =
            CCM_MOSAIC (_ccm_window_get_plugin (self->priv->areas[cpt].window,
                                                CCM_TYPE_MOSAIC));
        if (plugin)
        {
        ...
        cairo_matrix_translate (&transform,
                                (self->priv->areas[cpt].geometry.x -
                                 win_area.x) - (((win_area.width * scale) -
                                                 self->priv->areas[cpt].
                                                 geometry.width) / 2),
                                (self->priv->areas[cpt].geometry.y -
                                 win_area.y) - (((win_area.height * scale) -
                                                 self->priv->areas[cpt].
                                                 geometry.height) / 2));
...
ccm_mosaic_create_areas (CCMMosaic * self, gint nb_windows)
{
    ...
    for (i = 0; i < lines; i++)
    {
        n = MIN (nb_windows - self->priv->nb_areas,
                 ceilf ((float) nb_windows / lines));
        x = spacing;
        width =
...
ccm_mosaic_check_windows (CCMMosaic * self)
{
    ...
    if (self->priv->areas)
    {
        ...
        for (cpt = 0; cpt < self->priv->nb_areas; ++cpt)
        {
            ...
            {
                CCMMosaic *plugin =
                    CCM_MOSAIC (_ccm_window_get_plugin
                                (self->priv->areas[cpt].window,
                                 CCM_TYPE_MOSAIC));
                if (plugin)
                {
                ...
                ccm_drawable_pop_matrix (CCM_DRAWABLE
                                         (self->priv->areas[cpt].window),
                                         "CCMMosaic");
                ccm_mosaic_switch_keep_above (self,
                                              self->priv->areas[cpt].window,
                                              FALSE);
                g_object_set (G_OBJECT (self->priv->areas[cpt].window),
...
ccm_mosaic_on_option_changed (CCMPlugin * plugin, CCMConfig * config)
{
    ...
    if (config == ccm_mosaic_get_config (self, CCM_MOSAIC_SHORTCUT))
    {
        ...
        g_free (shortcut);
        g_signal_connect_swapped (ccm_mosaic_get_option (self)->keybind,
                                  "key_press",
...
static void
ccm_mosaic_preferences_page_init_desktop_section (CCMPreferencesPagePlugin *
                                                  plugin,
                                                  CCMPreferencesPage *
                                                  preferences,
                                                  GtkWidget * desktop_section)
{
    ...
    if (gtk_builder_add_from_file
        (self->priv->builder, UI_DIR "/ccm-mosaic.ui", NULL))
    {
        GtkWidget *widget =
        ...
        if (widget)
        {
            ...
            gtk_box_pack_start (GTK_BOX (desktop_section), widget, FALSE, TRUE,
                                0);
    ...
    ccm_preferences_page_plugin_init_desktop_section
...
ccm_mosaic_preferences_page_iface_init (CCMPreferencesPagePluginClass * iface)
{
    ...
    iface->init_desktop_section =
        ccm_mosaic_preferences_page_init_desktop_section;
