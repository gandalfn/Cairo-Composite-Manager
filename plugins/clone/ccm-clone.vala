...
namespace CCM
{
	class Output
	{
		...
		Output(CCM.Screen screen, CCM.Window window, X.Pixmap xpixmap, int depth)
		{
	...
	class Clone : CCM.Plugin, CCM.ScreenPlugin, CCM.WindowPlugin
	{
		...
		private void
		on_composite_message(CCM.Window window, long l1, long l2, long l3)
		{
			...
			if (atom == enable_atom)
			{
				...
				var clone = (Clone)window.get_plugin(typeof (Clone));
				var output = new Output(window.get_screen(), window, xpixmap, depth);
					...
					if (screen_outputs != null)
					{
						foreach (Output output in screen_outputs)
						{
								...
								output.pixmap.get_device_geometry_clipbox(out clipbox))
							{
								...
								{
									Cairo.Matrix matrix;
									matrix = Cairo.Matrix(clipbox.width / width,
														  0, 0,
